(()=>{
  ...
  const normalize=value=>{let p=String(value||'/').split('?')[0].split('#')[0];p=p.replace(/\/index\.html$/,'/').replace(/\.html$/,'').replace(/\/$/,'');return p||'/'};
  ...
  function desktopNav(){
    let nav=document.querySelector('.main-menu');if(!nav){const shell=document.querySelector('.shell');if(!shell)return;nav=document.createElement('nav');nav.className='main-menu';shell.querySelector('.site-header,.page-nav')?.after(nav)}
    ...
    const compiler=add('Compiler','/code');const builds=add('Builds','/builds');
  ...
  function mobileNav(){
    ...
    add('Home','/','⌂');const studio=document.createElement('button');studio.type='button';studio.className='create-tab'+(studioPaths.has(path)?' active':'');studio.innerHTML='<i>＋</i><span>Studio</span>';studio.setAttribute('aria-expanded','false');bar.append(studio);add('Code','/code','⌘');add('Builds','/builds','↻');const ota=document.createElement('a');ota.href='http://192.168.4.1';ota.innerHTML='<i>↗</i><span>OTA</span>';bar.append(ota);document.body.append(bar);
  ...
  function mark(root=document){const q=s=>root.querySelectorAll?.(s)||[];q('h1,h2').forEach(splitHeading);q('.home-hero,.feature,.card,.studio,.build-card,.empty,.lyric-editor,.code-card,.scene-panel,.preview-card,.latest-build,.projects-grid>*,.build-list>*').forEach((el,i)=>{if(el.dataset.appleMotion)return;el.dataset.appleMotion='surface';el.style.setProperty('--delay',Math.min(i%6,5)*42+'ms');observer().observe(el)});q('h3,h4,p,.brand small,.brand strong,.feature-link,.eyebrow,.hero-tag,.page-badge,.muted,.description,.hint,.note,footer span,label').forEach((el,i)=>{if(el.dataset.appleMotion)return;el.dataset.appleMotion='copy';el.style.setProperty('--delay',70+Math.min(i%7,6)*34+'ms');observer().observe(el)});q('.button,.settings-grid>*,.control-row>*,.code-grid>*,.studio-actions>*,.hero-actions>*,.editor-hints>*,.scene-row,.build-actions>*').forEach((el,i)=>{if(el.dataset.appleMotion)return;el.dataset.appleMotion='control';el.style.setProperty('--delay',Math.min(i%7,6)*28+'ms');observer().observe(el)});q('.section-title,.section-heading,.editor-title,.preview-head,.page-intro,.page-head').forEach(el=>{if(el.querySelector(':scope > .apple-rule'))return;const rule=document.createElement('span');rule.className='apple-rule';el.append(rule);observer().observe(rule)})}
  function ambient(){if(document.querySelector('.apple-orbit'))return;const x=document.createElement('div');x.className='apple-orbit';x.setAttribute('aria-hidden','true');x.innerHTML='<i></i><i></i><span></span>';document.body.append(x)}
