(()=>{
  if(window.__vallianAppleShellV5)return;
  window.__vallianAppleShellV5=true;

  const reduce=matchMedia('(prefers-reduced-motion: reduce)');
  const studioPaths=new Set(['/projects','/editor','/lyrics','/image','/draw','/video','/presets']);
  ...
  function desktopNav(){
    ...
    const compiler=add('Compiler','/code');const builds=add('Builds','/builds');const devices=add('Devices','/devices');
    const ota=document.createElement('a');ota.className='nav-ota';ota.href='http://192.168.4.1';ota.textContent='OTA';nav.append(ota);

    const setOpen=v=>{cluster.classList.toggle('open',v);trigger.setAttribute('aria-expanded',String(v))};
    ...
    const active=studioPaths.has(path)?trigger:({ '/':home,'/code':compiler,'/builds':builds,'/devices':devices }[path]);
    const move=(target,animate=true)=>{
      ...
      if(!animate)pill.style.transition='none';pill.style.opacity='1';pill.style.width=r.width+'px';pill.style.transform=`translateX(${r.left-nr.left}px)`;
      if(!animate)requestAnimationFrame(()=>pill.style.transition='');
    };
    requestAnimationFrame(()=>move(active,false));
    [...nav.children].filter(x=>x.matches('.nav-item,.nav-cluster')).forEach(x=>{const t=x.classList.contains('nav-cluster')?trigger:x;x.addEventListener('mouseenter',()=>move(t))});
  ...
  function mobileNav(){
    ...
    const sheet=document.createElement('div');sheet.className='mobile-studio-sheet';sheet.innerHTML='<div class="mobile-sheet-head"><strong>Studio</strong><span>TOOLS</span></div><div class="mobile-studio-grid"></div>';const grid=sheet.querySelector('.mobile-studio-grid');
    tools.forEach(([href,label,icon,sub])=>{const a=document.createElement('a');a.href=href;if(path===href)a.className='active';a.innerHTML=`<span class="studio-nav-icon">${icon}</span><span><b>${label}</b><small>${sub}</small></span>`;grid.append(a)});
  ...
  function observer(){
    ...
    io=new IntersectionObserver(entries=>entries.forEach(entry=>{
      const el=entry.target;el.style.setProperty('--motion-dir',String(direction));el.classList.toggle('is-in',entry.isIntersecting);
    }),{threshold:.07,rootMargin:'-2% 0px -4% 0px'});
    return io;
  }

  function splitHeading(el){
    if(!el||el.dataset.appleSplit)return;el.dataset.appleSplit='1';
    const text=el.innerHTML.replace(/<br\s*\/?\s*>/gi,'\n').replace(/<[^>]*>/g,'').trim();if(!text)return;
    el.setAttribute('aria-label',text.replace(/\s+/g,' '));el.innerHTML='';el.classList.add('apple-heading');
  ...
  function mark(root=document){
    ...
    q('.home-hero,.feature,.card,.studio,.build-card,.empty,.lyric-editor,.code-card,.scene-panel,.preview-card,.latest-build,.projects-grid>*,.build-list>*').forEach((el,i)=>{if(el.dataset.appleMotion)return;el.dataset.appleMotion='surface';el.style.setProperty('--delay',Math.min(i%6,5)*42+'ms');observer().observe(el)});
    q('h3,h4,p,.brand small,.brand strong,.feature-link,.eyebrow,.hero-tag,.page-badge,.muted,.description,.hint,.note,footer span,label').forEach((el,i)=>{if(el.dataset.appleMotion)return;el.dataset.appleMotion='copy';el.style.setProperty('--delay',70+Math.min(i%7,6)*34+'ms');observer().observe(el)});
    q('.button,.settings-grid>*,.control-row>*,.code-grid>*,.studio-actions>*,.hero-actions>*,.editor-hints>*,.scene-row,.build-actions>*').forEach((el,i)=>{if(el.dataset.appleMotion)return;el.dataset.appleMotion='control';el.style.setProperty('--delay',Math.min(i%7,6)*28+'ms');observer().observe(el)});
    q('.section-title,.section-heading,.editor-title,.preview-head,.page-intro,.page-head').forEach(el=>{if(el.querySelector(':scope > .apple-rule'))return;const rule=document.createElement('span');rule.className='apple-rule';el.append(rule);observer().observe(rule)});
  }

  function ambient(){
    if(document.querySelector('.apple-orbit'))return;
    const x=document.createElement('div');x.className='apple-orbit';x.setAttribute('aria-hidden','true');x.innerHTML='<i></i><i></i><span></span>';document.body.append(x);
  }
  ...
  function navigation(){
    document.addEventListener('click',e=>{
      const open=document.querySelector('.nav-cluster.open');if(open&&!e.target.closest('.nav-cluster'))open.classList.remove('open');
      const a=e.target.closest('a[href]');if(!a||e.defaultPrevented||a.hasAttribute('download')||(a.target&&a.target!=='_self'))return;
      const u=new URL(a.href,location.href);if(u.origin!==location.origin||u.href===location.href||reduce.matches)return;
      e.preventDefault();document.body.classList.add('route-leaving');setTimeout(()=>location.href=u.href,105);
    });
    addEventListener('pageshow',()=>{document.body.classList.remove('route-leaving');document.body.classList.add('page-entering');setTimeout(()=>document.body.classList.remove('page-entering'),760)});
  }

  function scrollState(){
    const y=Math.max(0,scrollY),d=y-lastY;if(Math.abs(d)>4){direction=d>0?1:-1;document.body.style.setProperty('--motion-dir',String(direction));document.body.classList.toggle('scroll-down',d>0&&y>100&&!document.body.classList.contains('studio-open'));lastY=y}
    const max=Math.max(1,document.documentElement.scrollHeight-innerHeight),p=Math.max(0,Math.min(1,y/max));document.body.style.setProperty('--scroll-p',p.toFixed(4));document.body.classList.toggle('motion-scrolled',y>24);
  }

  function init(){ensureStyle();desktopNav();mobileNav();ambient();mark();connection();navigation();scrollState();document.body.classList.add('page-entering');setTimeout(()=>document.body.classList.remove('page-entering'),760)}
  addEventListener('online',connection);addEventListener('offline',connection);
  addEventListener('scroll',()=>{if(ticking)return;ticking=true;requestAnimationFrame(()=>{ticking=false;scrollState()})},{passive:true});
  addEventListener('keydown',e=>{if(e.key!=='Escape')return;document.querySelector('.nav-cluster.open')?.classList.remove('open');document.querySelector('.mobile-studio-sheet.open')?.classList.remove('open');document.querySelector('.mobile-studio-backdrop.open')?.classList.remove('open');document.body.classList.remove('studio-open')});
  new MutationObserver(records=>{for(const rec of records)for(const node of rec.addedNodes)if(node.nodeType===1&&!node.matches?.('.mobile-tabbar,.mobile-studio-sheet,.mobile-studio-backdrop,.apple-orbit'))mark(node)}).observe(document.body,{childList:true,subtree:true});
  init();
})();
