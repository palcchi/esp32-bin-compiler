...
function readProfiles(){let list=[];try{const raw=JSON.parse(localStorage.getItem(PROFILES_KEY)||'[]');if(Array.isArray(raw))list=raw.map(profileClean).filter(x=>x.id&&x.id!=='default')}catch{}if(!list.length){const legacy=get();if(legacy.ssid!==DEFAULT.ssid||legacy.password!==DEFAULT.password)list=[profileClean({id:'saved-'+Date.now().toString(36),name:'Saved profile',ssid:legacy.ssid,password:legacy.password})]}
