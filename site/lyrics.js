const $=s=>document.querySelector(s),c=$('#lyricOled'),ctx=c.getContext('2d'),ta=$('#lyrics'),style=$('#style'),size=$('#size'),inv=$('#invertLyric'),status=$('#lyricStatus'),timeStatus=$('#timeStatus'),list=$('#parsedScenes'),count=$('#sceneCount');let timers=[],idx=0;
function parse(){return ta.value.split('\n').map((raw,lineIndex)=>({raw,lineIndex})).filter(v=>v.raw.trim()).map(({raw,lineIndex})=>{let parts=raw.split('|').map(v=>v.trim()),invert=false;if(parts.length&&/^(inv|invert)$/i.test(parts[parts.length-1])){invert=true;parts.pop()}let seconds=1.5;if(parts.length>1){const maybe=parseFloat(parts[parts.length-1]);if(Number.isFinite(maybe)){seconds=Math.max(.05,maybe);parts.pop()}}let sourceText=parts.join(' | ').trim(),text=/^\[pause\]$/i.test(sourceText)?'':sourceText;return{text,sourceText,seconds,invert,lineIndex}})}
function wrapWords(text,maxWidth){const words=text.trim().split(/\s+/).filter(Boolean),lines=[];let line='';const pushLong=word=>{let chunk='';for(const ch of word){const test=chunk+ch;if(chunk&&ctx.measureText(test).width>maxWidth){lines.push(chunk);chunk=ch}else chunk=test}if(chunk)line=chunk};for(const word of words){if(ctx.measureText(word).width>maxWidth){if(line){lines.push(line);line=''}pushLong(word);continue}const test=line?line+' '+word:word;if(line&&ctx.measureText(test).width>maxWidth){lines.push(line);line=word}else line=test}if(line)lines.push(line);return lines}
function drawWrapped(text,fontFamily,fontWeight='400',condense=1,maxLines=4){const sc=+size.value;let fontSize=style.value==='brat'?[16,20,24][sc-1]:style.value==='tiny'?[8,10,12][sc-1]:[11,14,17][sc-1],lines=[];const virtualWidth=122/condense;while(fontSize>=7){ctx.font=`${fontWeight} ${fontSize}px ${fontFamily}`;lines=wrapWords(text,virtualWidth);const lineH=fontSize*(style.value==='brat'?.88:1.05);if(lines.length<=maxLines&&lines.length*lineH<=58)break;fontSize--}if(lines.length>maxLines){const extra=lines.slice(maxLines-1).join(' ');lines=lines.slice(0,maxLines-1);lines.push(extra)}const lineH=fontSize*(style.value==='brat'?.88:1.05),startY=-(lines.length-1)*lineH/2;ctx.save();ctx.translate(64,32);ctx.scale(condense,style.value==='brat'?1.06:1);ctx.font=`${fontWeight} ${fontSize}px ${fontFamily}`;ctx.textAlign='center';ctx.textBaseline='middle';lines.forEach((line,i)=>ctx.fillText(line,0,startY+i*lineH,virtualWidth));ctx.restore()}
function draw(text='',invert=false){ctx.fillStyle=invert?'#e9fdff':'#000';ctx.fillRect(0,0,128,64);ctx.fillStyle=invert?'#001116':'#e9fdff';if(!text)return;if(style.value==='brat')drawWrapped(text.toLowerCase(),'Arial Narrow,Arial,sans-serif','400',.78,4);else if(style.value==='tiny')drawWrapped(text,'ui-monospace,monospace','400',1,5);else drawWrapped(text,'Arial,sans-serif','600',1,4)}
