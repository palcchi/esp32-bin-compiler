...
export default async function handler(req,res){
  ...
  try{
    ...
    const payload=JSON.stringify({style,size,createdAt:new Date().toISOString(),estimatedFrames:estimated,scenes},null,2)+'\n';
    ...
    return send(res,202,{ok:true,commit:data.commit.sha,short:data.commit.sha.slice(0,7),estimatedFrames:estimated,message:'Build queued'});
