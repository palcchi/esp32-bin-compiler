...
export default async function handler(req,res){
  ...
  try{
    ...
    const animation=['none','pop','slide','shake'].includes(body?.animation)?body.animation:'none';
    ...
    const maxScenes=animation==='none'?120:55;
    if(incoming.length>maxScenes) return send(res,400,{error:`Mode ${animation} maksimal ${maxScenes} scene agar firmware tetap muat di partisi OTA.`});
    const scenes=incoming.map((s,i)=>({text:String(s?.text??'').slice(0,220),duration:Math.max(50,Math.min(120000,Math.round(Number(s?.duration)||1500))),invert:!!s?.invert,order:i}));
    const payload=JSON.stringify({style,animation,size,createdAt:new Date().toISOString(),scenes},null,2)+'\n';
    ...
    const update=await fetch(`https://api.github.com/repos/${REPO}/contents/${PATH}`,{method:'PUT',headers:{...headers,'Content-Type':'application/json'},body:JSON.stringify({message:`lyrics: web build ${new Date().toISOString()}`,content:Buffer.from(payload).toString('base64'),sha:currentData.sha,branch:'main'})});
